...
class _WeatherScreenState extends State<WeatherScreen> {
  ...
  String addMinutesToStringDate(String dateString, int minutesToAdd) {
    // Parse the string to a DateTime object
    DateTime dateTime = DateFormat('yyyy-MM-dd HH:mm').parse(dateString);

    // Add the specified duration
    DateTime newTime = dateTime.add(Duration(minutes: minutesToAdd));

    // Format the new DateTime object back to a string
    return DateFormat('yyyy-MM-dd HH:mm').format(newTime);
  }
...
String getSunFormattedTime(int timestamp) {
  ...
  DateTime newTime = dateTime.add(Duration(hours: 4, minutes: 15));
  return DateFormat('HH:mm').format(newTime);
}
